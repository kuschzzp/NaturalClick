...
Status: implemented for the 0.4.x runtime. The current documented extension version is `0.4.306`, with the source version tracked in `naturalclick-extension/manifest.json`.

## Goal

NaturalClick 0.4.x moves repeatable browser automation steps out of model-only planning and into deterministic workflows. The model remains useful for ambiguous pages, but routine search testing, form-field recovery, and model-timeout recovery should not depend on guessing the next field, inventing dropdown options, or clicking target-page navigation as a fallback.
...
The pre-model order is target URL, login, task navigation, then search-field testing. Timeout recovery order is unresolved task navigation, then constrained form-fill recovery. Search testing only runs after named task navigation targets are reached or when the task has no named page/module target.
...
Collapsed search panels are expanded first. Text fields are filled with deterministic test values derived from visible table/list samples or explicit task values. Selection fields are opened first; the workflow chooses only real candidates returned by the content action outcome or exposed field option labels. After each field is submitted, the workflow resets filters before moving to the next field. If no reset action is observed, the workflow may clear only ordinary editable text fields by setting their value to empty; selection, date, and multi-select controls still require more context instead of guessed clearing.

## Form Timeout Recovery

When the text model times out on a create/add form, the form-fill workflow may recover only if the current page/module target is already reached and the observed structured form fields can be matched to explicit task assignments.
...
- Never test a generic search area while a named page/module target is still unresolved.
- Never add site- or domain-specific fallback logic; recovery policies must remain generic browser-operation policies.
- Model timeout recovery may stop with a clear failure, but it must not click target navigation as a blind fallback, reopen a create entry as a submit action, or continue search tests on the wrong page.
