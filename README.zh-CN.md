...
| 选择控件 | 下拉框、复选项、单选项、树节点、Element Plus 风格下拉项 |
| 级联选择器 | 父级悬浮展开下一级，叶子节点点击完成选择 |
| 跨标签页动作 | 打开、切换、关闭标签页 |
| 视觉回退 | 基于截图的坐标定位，并在执行前做命中检测 |
| 轨迹检查 | 模型 IO、动作输入、输出、校验失败、导出的会话日志 |
...
| 选择控件 | 更确定的多选、树选择、级联选择器行为 |
| 结果校验 | 更清晰的执行前后观察差异和点击结果校验 |
