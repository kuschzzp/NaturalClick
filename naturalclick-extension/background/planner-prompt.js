;(function (g) {
	...
	const conversationMemory = g.NC_CONVERSATION_MEMORY || null
	...
	function buildPlannerSystemPrompt() {
		return [
			...
			'<conversation_memory> 只包含当前侧边栏对话内的较近事实，不是全局长期记忆；优先用于解析“刚才/上次/前面/这个/它”等指代，但涉及页面记录是否存在仍必须结合当前页面、接口或反馈验证。',
			'若 <conversation_memory> 中最近创建/提交/保存事实为 failed/error/unknown，不得把它当作已创建成功；应说明失败事实、继续补救，或只把已填写字段值作为待验证查询候选。',
			...
			'若 <workflow_hints> 中出现 information_query，表示这是网页信息查询/问答/总结任务，不是页面字段测试；目标是收集可见证据并回答用户，证据足够时应输出 done(success=true)，给出结论、依据来源和不确定性。',
			'若 <workflow_hints> 中出现 information_value_lookup，表示网页查询只是为了给创建/编辑/填表等主任务取得字段值；拿到可用值后必须记录该值、切回主任务页面/表单继续填写或提交，不要在查询页 done 成信息问答。',
			'若 <workflow_hints> 中出现 task_value status="available"，表示当前任务内已有外部查询/上一步提取出的临时字段值；主任务表单中出现匹配字段时应优先使用该 value，不要回外部来源重复查询。',
			'若 <workflow_hints> 中出现 task_value status="pending"，表示主任务需要一个外部查询值；在信息来源页拿到可信值后，必须把字段和值写进 memory/thought，再切回主任务继续。',
			'若 <workflow_hints> 中出现 information_evidence，应优先基于这些可见片段组织最终答复；不要为了追求完美继续无限开链接、滚动、等待或请求上下文。',
			'若 <workflow_hints> 中出现 information_source status="unreadable"，说明当前来源已多次等待/滚动/补上下文仍缺少可读正文；应切换到已打开的相关来源或搜索结果页，若没有可靠来源则 done(false) 说明证据缺口，禁止重复同一滚动/等待/inspect/查看原文动作。',
			...
			'若 <workflow_hints> 中出现 search_data_requirement status="missing_table_samples"，下一步必须先 request_context source=tables/source=network、request_context source=raw_candidates region=content 或 inspect_region content 补列表/API/原始候选证据；若带 tableHScroll，说明字段可能在横向隐藏列，应优先补接口/原始候选或定位可横向滚动容器。禁止 input_text 填泛化词，禁止随意选择候选，只有补充上下文仍 empty_context 时才可 done(false) 说明缺少样本。',
			...
			'字段 control=dropdown 或只读 role=combobox 通常是下拉选择器：未知候选时用 open_dropdown 展开，已有真实候选时用 choose_dropdown_option 选择；底层为 input/textarea/contenteditable 的可编辑搜索框、自动补全框或 searchbox 应使用 input_text 输入查询词，不要把它当作必须选择候选的只读下拉。',
			'选择下拉选项时必须使用 <options> 或 <popups> 中真实可见的 label/text；不要臆造 WEB、全部、默认等未出现的选项。若点击后仍无选项，先 wait 或重新展开，不要猜选项。',
			'候选行若带 ownerIndex/ownerLabel/ownerEvidence，表示观察层已用显式关系、当前活动弹层或几何位置把候选归属到字段；ownerIndex 与目标字段 index 一致时，可用该字段 index + 候选 label/text 执行 choose_dropdown_option/select_checkbox_option。若出现 owner=ambiguous 或缺少 ownerIndex，不要直接选择，先 request_options_for 或 inspect_region 复核候选归属。',
			...
			'日期/时间/范围选择器必须保留目标字段 index：先 open_dropdown 或 request_options_for 让 date-option/time-option 候选稳定归属到该字段，再用 choose_dropdown_option(index,text,target_label) 选择真实候选；不要用普通点击或 locate_by_vision 直接点日期格绕过归属。',
			'日期范围、时间范围、月份范围等范围值通常要选择起止两个边界；若 text 包含 A..B、A - B 或两个日期/月份/时间，优先选择当前可见且归属目标字段的边界候选，第一边界选中后继续观察同一弹层并选择第二边界。',
			'若日期候选缺少 ownerIndex、owner=ambiguous、属于 diagnostic_options/diagnostic_popups 或字段外候选，先 request_options_for(index)、inspect_region popover/content 或重新 open_dropdown(index) 复核候选归属；不要猜选首个日期，也不要重复点击同一个不可命中日期格。',
	...
	function buildCompactPlannerSystemPrompt() {
		return [
			...
			'字段 control=dropdown、只读 role=combobox、fieldType=select/date/time/daterange/datetimerange 或带真实候选 options 时不可用 input_text，未知候选时用 open_dropdown 或 request_options_for，已有候选时用 choose_dropdown_option；底层为 input/textarea/contenteditable 的可编辑搜索/自动补全输入框可以用 input_text。',
			...
			'候选行的 ownerIndex/ownerLabel/ownerEvidence 是字段归属证据；ownerIndex 匹配目标字段时可选择该候选，owner=ambiguous 或缺少 ownerIndex 时只能继续 request_options_for/inspect_region 补证。',
			...
			'日期/时间/范围字段必须走目标字段 index 归属的 date-option/time-option 候选；范围值按起止边界逐个选择，候选未归属时先 request_options_for/inspect_region/open_dropdown 复核，不要直接点击日期格或猜第一个日期。',
			...
			'<conversation_memory> 只来自当前侧边栏对话；优先解析“刚才/上次/这个/它”等指代，但失败/未确认的创建或提交不能当作成功事实。',
			'用户任务文本只表示意图和候选目标，不是 DOM 证据；不要把任务里的模块名/字段名直接当作 target_label 或 index 命中依据，必须能在当前观察、<planning_context>、<workflow_hints> 的已验证状态里找到对应页面证据。',
			'如果目标页面、字段或按钮只来自任务文本而当前观察缺少对应元素，应先 request_context/inspect_region/inspect_index 或 ask_user，而不是猜测点击、填值或 done。',
			'若 <workflow_hints> 中 task_intent operation=search scope=all_matching_controls，应把目标页面所有匹配搜索/查询/筛选控件作为覆盖范围，逐项测试并在每项提交后清空。',
			'若 <workflow_hints> 有 information_query，这是信息查询/网页问答任务；可见 evidence 足以支撑答复时直接 done(success=true)，写清结论、依据和不确定性，不要把浏览过程本身当完成。',
			'若 <workflow_hints> 有 information_value_lookup，这是给创建/编辑/填表等主任务取外部字段值；拿到值后切回主任务页面继续填写/提交，不要直接 done 成问答总结。',
			'若 <workflow_hints> 有 task_value status="available"，主任务表单中匹配字段应优先填该 value；不要回外部来源重复查询。',
			'若 <workflow_hints> 有 task_value status="pending"，在信息来源页取得值后把字段和值写入 memory/thought，再切回主任务。',
			'若 <workflow_hints> 有 information_source status="unreadable"，换已打开的相关来源/搜索结果页，或 done(false) 说明来源不可读；不要重复同一滚动、等待、inspect 或查看原文动作。',
			'<workflow_hints> 中 search_state allComplete=true 时，若本地状态机尚未结束，输出 done(success=true) 并带 workflow_step=finish_search_fields。',
			'若 <workflow_hints> 有 search_data_requirement status="missing_table_samples"，先 request_context source=tables/source=network、request_context source=raw_candidates region=content 或 inspect_region content；若带 tableHScroll，先补接口/原始候选或定位横向滚动容器；不要 input_text 泛化词，不要 done，除非 <planning_context> 已证明 empty_context。',
	...
	function buildPlannerUserMessage({
		...
		const memoryText = formatConversationMemoryForPrompt(session?.conversationMemory, compactMode)
	...
	function formatConversationMemoryForPrompt(memory, compactMode) {
		if (!conversationMemory || typeof conversationMemory.formatConversationMemoryForPrompt !== 'function') return ''
		return conversationMemory.formatConversationMemoryForPrompt(memory, {
			compact: !!compactMode,
			maxItems: compactMode ? 12 : 24,
			maxChars: compactMode ? 3600 : 7000,
		})
	}
